...
from GraphEmbedding.inits import zeros
from GraphEmbedding.layers import Layer
...
class BipartiteEdgePredLayer(Layer):
    ...
    def _xent_loss(self, inputs1, inputs2, neg_samples, hard_neg_samples=None):
        aff = self.affinity(inputs1, inputs2) #连接关系分数 相似度计算
        neg_aff = self.neg_cost(inputs1, neg_samples, hard_neg_samples) #和负例的每个分别计算
        true_xent = tf.nn.sigmoid_cross_entropy_with_logits(
                labels=tf.ones_like(aff), logits=aff)
        negative_xent = tf.nn.sigmoid_cross_entropy_with_logits(
                labels=tf.zeros_like(neg_aff), logits=neg_aff)
        loss = tf.reduce_sum(true_xent) + self.neg_sample_weights * tf.reduce_sum(negative_xent) #正例+负例的交叉熵和
        return loss
