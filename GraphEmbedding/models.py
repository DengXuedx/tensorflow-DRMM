...
import GraphEmbedding.layers as layers
import GraphEmbedding.metrics as metrics

from GraphEmbedding.prediction import BipartiteEdgePredLayer
from GraphEmbedding.aggregators import MeanAggregator, MaxPoolingAggregator, MeanPoolingAggregator, SeqAggregator, GCNAggregator
...
class SampleAndAggregate(GeneralizedModel):
    ...
    def __init__(self, placeholders, features, adj, degrees,
            ...
            model_size="small", identity_dim=0,loss_fn = 'xent',
            **kwargs):
        '''
        Args:
        ...
        if features is None: 
            ...
            self.features = tf.Variable(tf.constant(features, dtype=tf.float32), trainable=False, name="node_features")
    ...
    def sample(self, inputs, layer_infos, batch_size=None):
        ...
        for k in range(len(layer_infos)):
            ...
            samples.append(tf.reshape(node, [support_size * batch_size,])) #sample neighbors
        ...
        for layer in range(len(num_samples)):
            ...
            for hop in range(len(num_samples) - layer):
                ...
                neigh_dims = [batch_size * support_sizes[hop], 
                              ...
                              dim_mult*dims[layer]] #(-1,num_sample,106)
                h = aggregator((hidden[hop],
                                tf.reshape(hidden[hop + 1], neigh_dims))) #(-1,256)
    ...
    def _build(self):
        labels = tf.reshape(
                ...
                [self.batch_size, 1])  #skip-gram real output word
        with tf.name_scope("Negative_Sampling"):
            self.neg_samples, _, _ = (tf.nn.fixed_unigram_candidate_sampler(
                ...
                distortion=0.75,
                unigrams=self.degrees.tolist())) #neg sample outputword

           
        # perform "convolution"
        with tf.name_scope("Sampling_Neighbor_Nodes"):
        ...
        with tf.name_scope("Aggregate_Neighbor_Features"):
        ...
        with tf.name_scope("BipartiteEdge_Predication"):
            self.link_pred_layer = BipartiteEdgePredLayer(dim_mult*self.dims[-1],
                    dim_mult*self.dims[-1], self.placeholders, act=tf.nn.sigmoid, loss_fn=self.loss_fn,
    ...
    def _loss(self):
        with tf.name_scope("Pairwise_Loss"):
            for aggregator in self.aggregators:
                for var in aggregator.vars.values():
                    self.loss += FLAGS.weight_decay * tf.nn.l2_loss(var)
        with tf.name_scope("BipartiteEdgePred_Loss"):
            self.loss += self.link_pred_layer.loss(self.outputs1, self.outputs2, self.neg_outputs) 
            tf.summary.scalar('loss', self.loss)

    def _accuracy(self):
        # shape: [batch_size]
        with tf.name_scope("affinity_score"):
        ...
        with tf.name_scope("affinity_rank"):
            _, indices_of_ranks = tf.nn.top_k(self.aff_all, k=size) #sort indices
            _, self.ranks = tf.nn.top_k(-indices_of_ranks, k=size) # rank 
        with tf.name_scope("edge_pred_mrr"):
...
class Node2VecModel(GeneralizedModel):
    def __init__(self, placeholders, dict_size, degrees, name=None,
                 nodevec_dim=50, lr=0.001,loss_fn = 'xent', **kwargs):
    ...
    def _build(self):
        ...
        self.link_pred_layer = BipartiteEdgePredLayer(self.hidden_dim, self.hidden_dim,
                self.placeholders,loss_fn=self.loss_fn, bilinear_weights=False)
